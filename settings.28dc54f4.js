...
// Wire op dropdowns to show/hide inputs
for (const f of MD_FIELDS) {
...
mdEl('saveMyDayBtn').addEventListener('click', () => {
  ...
  // Re-apply good-day highlights to current forecast
...
// Load form when tab is opened
const _origSwitchTab = switchSettingsTab;


// ── Cloud Backup ──────────────────────────────────────────────────────────────
const CLOUD_API = 'https://wind-backup.YOUR_SUBDOMAIN.workers.dev/backup';
const CLOUD_UID_KEY = 'wind_cloud_uid_v1';

function getCloudUid() {
  return localStorage.getItem(CLOUD_UID_KEY) || null;
}

function saveCloudUid(uid) {
  localStorage.setItem(CLOUD_UID_KEY, uid.toLowerCase());
}
...
function cloudStatus(msg, ok = null) {
  const el = document.getElementById('cloudStatus');
  if (!el) return;
  el.style.display = 'block';
  el.textContent = msg;
  el.style.background = ok === true ? '#d4edda' : ok === false ? '#f8d7da' : '#fff3cd';
  el.style.color     = ok === true ? '#155724' : ok === false ? '#721c24' : '#856404';
}

function updateUidDisplay() {
  const el = document.getElementById('cloudUidDisplay');
  if (!el) return;
  const uid = getCloudUid();
  el.textContent = uid ? uid.toUpperCase() : '—';
}

async function checkUidAvailable(uid) {
  try {
    const r = await fetch(`${CLOUD_API}/${uid}`);
    const d = await r.json();
    return d.exists === false; // true = available
  } catch { return null; } // null = network error
}

async function ensureUid() {
  let uid = getCloudUid();
  if (uid) return uid;
  // Generate until we find one not taken
  for (let i = 0; i < 10; i++) {
    const candidate = generateUid();
    cloudStatus('Checking availability…');
    const available = await checkUidAvailable(candidate);
    if (available === null) { cloudStatus('Network error — cannot check availability.', false); return null; }
    if (available) {
      saveCloudUid(candidate);
      updateUidDisplay();
      return candidate;
    }
  }
  cloudStatus('Could not find a free code — try again.', false);
  return null;
}

document.getElementById('cloudGenNewBtn')?.addEventListener('click', async () => {
  // Force generate a new uid (warn if already have one)
  const existing = getCloudUid();
  if (existing) {
    if (!confirm(`You already have code ${existing.toUpperCase()}. Generate a new one?\n\nYour old code will still work to restore, but future saves will use the new code.`)) return;
  }
  localStorage.removeItem(CLOUD_UID_KEY);
  updateUidDisplay();
  cloudStatus('Generating new code…');
  await ensureUid();
  cloudStatus('');
});
...
document.getElementById('cloudSaveBtn')?.addEventListener('click', async () => {
  ...
  try {
    ...
    if (r.ok) {
      cloudStatus(`✅ Saved! Your code: ${uid.toUpperCase()} — use it to restore on any device.`, true);
    } else {
      cloudStatus(`❌ Server error (${r.status}). Try again.`, false);
    }
  } catch (e) {
    cloudStatus('❌ Network error. Check your connection.', false);
  }
});

document.getElementById('cloudRestoreBtn')?.addEventListener('click', async () => {
  const input = document.getElementById('cloudRestoreCode');
  const code = (input?.value || '').trim().toLowerCase();
  if (code.length !== 5 || !/^[a-z]+$/.test(code)) {
    cloudStatus('Please enter a valid 5-letter code.', false); return;
  }
  cloudStatus('Restoring…');
  try {
    const r = await fetch(`${CLOUD_API}/${code}`);
    const d = await r.json();
    if (d.exists === false) {
      cloudStatus('❌ No data found for that code.', false); return;
    }
    // Reuse import logic
    ...
    saveCloudUid(code);
    updateUidDisplay();
    cloudStatus(`✅ Restored from code ${code.toUpperCase()}!`, true);
  } catch (e) {
    cloudStatus('❌ Network error. Check your connection.', false);
  }
});

// Init: show uid on load
updateUidDisplay();
