...
if(ev.key==='Escape'){committed=true;buildFoldersUI();}
});
input.addEventListener('blur',()=>setTimeout(commitRename,100));
...
const normalized=normalizeGroupName(newName);
if(normalized==='Ungrouped')return;
const groups=getAllGroups();
if(groups.includes(normalized)&&normalized!==oldName)return;
const places=getPlaces();
let changed=false;
for(const p of places){
if(normalizeGroupName(p.group)===oldName){
...
try{
const saved=JSON.parse(localStorage.getItem('wind_folder_order_v1')||'[]');
const updated=saved.map(n=>n===oldName?normalized:n);
localStorage.setItem('wind_folder_order_v1',JSON.stringify(updated));
}catch(e){}
reloadPlacesKeepSelection();
buildFoldersUI();
refreshSelectedPanel();
if(typeof populateFolderWaveCalibrationList==='function'){
