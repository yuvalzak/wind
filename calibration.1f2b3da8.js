...
let editingRuleIndex = null; // null = adding new, number = updating existing
...
function openCalibration() {
  ...
  // Close settings modal if open (calibration is launched from within it)
  ...
  rules.forEach((rule, idx) => {
    ...
    html += `
      <div 
        ...
        onclick="loadRuleIntoForm(${idx})"
        title="Tap to load this rule into the editor"
...
function resetAddRuleBtn() {
  editingRuleIndex = null;
  addCalibrationRule.textContent = '➕ Add Rule';
  addCalibrationRule.style.background = '';
}
...
function loadRuleIntoForm(idx) {
  ...
  // Load sectors
  selectedSectors = [...rule.sectors];

  // Load operation and type
  const isNeg = rule.value < 0;
  setCalOperation(isNeg ? 'subtract' : 'add');
  setCalType(rule.type);

  // Load value (always positive in selector)
  const absVal = Math.abs(rule.value);
  const opts = calValue.options;
  let matched = false;
  for (let i = 0; i < opts.length; i++) {
    if (parseFloat(opts[i].value) === absVal) { calValue.selectedIndex = i; matched = true; break; }
  }
  if (!matched && opts.length > 0) calValue.selectedIndex = 0;

  // Load wind condition
  if (rule.windCondition) {
    calWindEnabled.checked = true;
    calWindCondition.disabled = false;
    calWindThreshold.disabled = false;
    calWindCondition.value = rule.windCondition.type;
    calWindThreshold.value = rule.windCondition.threshold;
  } else {
    calWindEnabled.checked = false;
    calWindCondition.disabled = true;
    calWindThreshold.disabled = true;
  ...
  // Load gust condition
  if (rule.gustCondition) {
    calGustEnabled.checked = true;
    calGustCondition.disabled = false;
    calGustThreshold.disabled = false;
    calGustCondition.value = rule.gustCondition.type;
    calGustThreshold.value = rule.gustCondition.threshold;
  } else {
    calGustEnabled.checked = false;
    calGustCondition.disabled = true;
    calGustThreshold.disabled = true;
  ...
  // Load wave condition
  if (rule.waveCondition) {
    calWaveEnabled.checked = true;
    calWaveCondition.disabled = false;
    calWaveThreshold.disabled = false;
    calWaveCondition.value = rule.waveCondition.type;
    calWaveThreshold.value = rule.waveCondition.threshold;
  } else {
    calWaveEnabled.checked = false;
    calWaveCondition.disabled = true;
    calWaveThreshold.disabled = true;
  ...
  addCalibrationRule.textContent = '✏️ Update Rule';
  addCalibrationRule.style.background = '#e67e22';
  drawCompass();
}
window.loadRuleIntoForm = loadRuleIntoForm;
