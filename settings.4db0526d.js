...
function populateFolderHistoryList() {
  ...
  for (const entry of history) {
    ...
    // Spots list with individual checkboxes
    const spotsLabel = document.createElement('div');
    spotsLabel.style.cssText = 'font-size: 0.85em; color: #5d6d7e; font-weight: 600; margin-bottom: 8px;';
    spotsLabel.textContent = 'Or select individual spots to restore:';

    const spotsGrid = document.createElement('div');
    spotsGrid.style.cssText = 'display: grid; gap: 6px; margin-bottom: 12px;';

    for (const sp of entry.spots) {
      const row = document.createElement('label');
      row.style.cssText = 'display: flex; align-items: center; gap: 8px; padding: 6px 10px; background: white; border-radius: 6px; cursor: pointer; border: 1px solid #dee2e6;';
    ...
    restoreSelectedBtn.style.cssText = 'padding: 7px 14px; background: #3498db; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 700; font-size: 0.9em;';
    ...
    card.appendChild(spotsGrid);
    card.appendChild(restoreSelectedBtn);
    container.appendChild(card);
  }
}
// v1771297632
