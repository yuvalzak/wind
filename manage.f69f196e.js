...
const dataModalSpotName=document.getElementById('dataModalSpotName');
function refreshDataButton(){
const loc=PLACES[currentIndex];
if(!loc){
dataBtn.style.display='none';
dataSetBtn.style.opacity='0.5';
return;
}
const url=getDataUrl(loc.key);
if(url){
dataBtn.style.display='flex';
dataBtn.title='Open real-time data:'+url;
dataSetBtn.style.opacity='1';
dataSetBtn.title='Edit real-time data URL';
}else{
dataBtn.style.display='none';
dataSetBtn.style.opacity='0.5';
dataSetBtn.title='Set real-time data URL(Windguru,Windy…)';
}
}
...
const loc=PLACES[currentIndex];
if(!loc)return;
dataModalSpotName.textContent=loc.name;
dataUrlInput.value=getDataUrl(loc.key)||'';
dataModal.style.display='flex';
setTimeout(()=>dataUrlInput.focus(),50);
});
document.getElementById('dataSaveBtn').addEventListener('click',()=>{
const loc=PLACES[currentIndex];
if(!loc)return;
let url=dataUrlInput.value.trim();
if(url&&!url.startsWith('http'))url='https://'+url;
saveDataUrl(loc.key,url);
dataModal.style.display='none';
refreshDataButton();
});
document.getElementById('dataClearBtn').addEventListener('click',()=>{
const loc=PLACES[currentIndex];
if(!loc)return;
saveDataUrl(loc.key,'');
dataUrlInput.value='';
...
if(e.key==='Enter')document.getElementById('dataSaveBtn').click();
});