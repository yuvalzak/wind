...
function dayBlock(dayLabel, data, locName) {
  ...
  return `
    <div class="day-separator" data-day-id="${id}">${locName} • ${dayLabel}</div>
      ...
      plugins: {
        ...
        tooltip: {
          ...
          external: (context) => {
            ...
            windEl.textContent = `💨 ${d.speed}k`;
            windEl.style.color = measured ? '#27ae60' : '';
            gustEl.textContent = `↑${d.gust}k`;

            // Wave
            if (typeof d.waveH === 'number') {
              const pTxt = (typeof d.waveP === 'number') ? ` ${d.waveP.toFixed(1)}s` : '';
              const dirTxt = (typeof d.waveDir === 'number') ? ` ${degToCardinal(d.waveDir)}` : '';
              waveEl.textContent = `🌊 ${d.waveH.toFixed(1)}m${pTxt}${dirTxt}`;
            } else {
              waveEl.textContent = '';
            }
