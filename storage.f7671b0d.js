...
if(lsGet(STORAGE_PLACES))return;
...
function hourMatchesMyDay(prefs,{speed,gust,waveH,waveP,precip}){
...
if(prefs.no_rain&&precip!=null&&precip>0.1)return false;
return(
...
if(!prefs)return false;
const fields=['wind','gust','waveH','waveP'];
const hasActiveRule=fields.some(f=>prefs[f+'_op']&&prefs[f+'_op']!=='any')||!!prefs.no_rain;
...
const minHours=(typeof prefs.min_hours==='number'&&prefs.min_hours>0)
?prefs.min_hours:null;
const matchingHours=dayHours.filter(h=>hourMatchesMyDay(prefs,h)).length;
if(minHours!==null){
return matchingHours>=minHours;
}
return matchingHours===dayHours.length;
}
function getCamUrl(key){
const p=getPlaces().find(p=>p.key===key);
return p?(p.camUrl||''):'';
}
function saveCamUrl(key,url){
const places=getPlaces();
const idx=places.findIndex(p=>p.key===key);
if(idx<0)return;
places[idx].camUrl=url;
savePlaces(places);
}
...
function saveDataUrl(key,url){
const places=getPlaces();
const idx=places.findIndex(p=>p.key===key);
if(idx<0)return;
places[idx].dataUrl=url;
savePlaces(places);
}