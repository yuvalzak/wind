...
calWaveEnabled.addEventListener('change', () => {
  const enabled = calWaveEnabled.checked;
  calWaveCondition.disabled = !enabled;
  calWaveThreshold.disabled = !enabled;
});
...
addCalibrationRule.addEventListener('click', () => {
  ...
  if (calWaveEnabled.checked) {
    rule.waveCondition = {
      type: calWaveCondition.value, // 'less' or 'greater'
...
function renderCalibrationRules() {
  ...
  rules.forEach((rule, idx) => {
    ...
    if (rule.waveCondition) {
      const condType = rule.waveCondition.type === 'less' ? '<' : '>';
      conditionText = ` <span style="color: #16a085; font-size: 0.85em;">(waves ${condType} ${rule.waveCondition.threshold}m)</span>`;
    }
