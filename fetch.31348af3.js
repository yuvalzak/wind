  ...
  await Promise.allSettled(
    PLACES.map(async (loc) => {
      try {
        ...
        const visibility = (typeof wc.visibility === 'number') ? (wc.visibility / 1000).toFixed(1) : null; // Convert m to km
        ...
        try {
          ...
          if (mr.ok) {
            ...
            // Water temperature not available in current marine API
            waterTemp = null;
          }
        } catch {}
        
        // Apply calibration rules with wave height
        const waveHeight = waveH ? parseFloat(waveH) : null;
        wind = applyCalibration(wind, dir, loc.key, waveHeight);
        gust = applyCalibration(gust, dir, loc.key, waveHeight);
        ...
        currentCache[loc.key] = { wind, gust, dir: Math.round(dir), card, timeStr, waveH, waveP, waveDir, airTemp, waterTemp, aqi, visibility, cloudCover, lightning };
      ...
      if (wc && typeof wc.windspeed_10m === 'number') {
        ...
        currentCache[loc.key] = {
          ...
          timeStr: windData.current?.time
            ...
            : '',
          waveH, waveP, waveDir, airTemp, waterTemp, aqi, visibility, cloudCover, lightning
        };
        updateButtonRealtime(loc.key);
