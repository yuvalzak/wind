...
function openMap() {
  ...
  if (isEditMode && loc) {
    mapModalTitle.textContent = `Edit location: ${loc.name}`;
    mapModalHint.textContent = 'Drag the blue marker or click a new location → Update';
  } else {
    mapModalTitle.textContent = 'Pick a location';
    mapModalHint.textContent = 'Click map → name it → choose folder → Save';
  }
  ...
  if (!mapInitialized) {
    ...
    // Start with satellite + labels by default
  ...
  if (loc) {
    ...
    if (currentSpotMarker) {
      ...
      currentSpotMarker.on('dragend', function(e) {
        ...
        // Set up for saving the new location
...
savePointBtn.addEventListener('click', async () => {
  ...
  // Check if we're updating an existing spot (based on button text)
  const isUpdate = savePointBtn.textContent === 'Update Location';
  
  if (isUpdate && PLACES[currentIndex]) {
    // Update existing spot location
    const existingKey = PLACES[currentIndex].key;
    const placeIdx = places.findIndex(p => p.key === existingKey);
  ...
  newFolderInputEl.value = '';
  savePointBtn.disabled = true;
  savePointBtn.textContent = 'Save Spot'; // Reset button text
