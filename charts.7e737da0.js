...
function drawDay(dayObj) {
  ...
  for (let i = 0; i < data.length; i++) {
    const o = data[i];
    // Normalize direction to 0-360 range (handle any values over 360)
    const normalizedDir = ((o.dir % 360) + 360) % 360;
    const card = degToCardinal(normalizedDir);
    
    // Use direction directly - the arrow shows wind direction as-is
    // In CSS: 0° is up (north), 90° is right (east), 180° is down (south), 270° is left (west)
    
    dRow.insertAdjacentHTML('beforeend', `
      <div class="direction-item" style="width:${calculatedItemW}px">
        <div class="arrow" style="transform:rotate(${normalizedDir}deg)">↓</div>
        <div class="cardinal">${card}</div>
        <div class="degree">${Math.round(normalizedDir)}°</div>
      </div>
    `);
...
function renderForecast(windData, marineData, loc) {
  ...
  for (let i = 0; i < h.time.length; i++) {
    ...
    const speed = applyCalibration(Math.round(rawSpeed), dir, loc.key);
    const gust = applyCalibration(Math.round(rawGust), dir, loc.key);
