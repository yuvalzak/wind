...
if(!parsed||parsed.lat<-90||parsed.lat>90||parsed.lon<-180||parsed.lon>180){
latLonInput.style.borderColor='#e74c3c';
latLonInput.style.background='#fff5f5';
setTimeout(()=>{latLonInput.style.borderColor='';latLonInput.style.background='';},1500);
return;
}
...
pickedCoordsEl.textContent=`Lat/Lon:${lat.toFixed(5)},${lon.toFixed(5)}`;
savePointBtn.disabled=false;
latLonInput.style.borderColor='#27ae60';
setTimeout(()=>{latLonInput.style.borderColor='';},1000);
if(document.getElementById('placeName').value===''){
document.getElementById('placeName').focus();
}
}
goToLatLonBtn.addEventListener('click',goToLatLon);
latLonInput.addEventListener('keydown',(e)=>{if(e.key==='Enter')goToLatLon();});